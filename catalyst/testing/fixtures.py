...
from catalyst.pipeline import SimplePipelineEngine
from catalyst.pipeline.data import USEquityPricing
from catalyst.pipeline.loaders import USEquityPricingLoader
...
class WithAssetFinder(WithDefaultDateBounds):
    ...
    See Also
    --------
    catalyst.testing.make_simple_equity_info
    catalyst.testing.make_jagged_equity_info
    catalyst.testing.make_rotating_equity_info
    catalyst.testing.make_future_info
    catalyst.testing.make_commodity_future_info
    """
...
class WithTradingEnvironment(WithAssetFinder,
    ...
    See Also
    --------
    :class:`catalyst.finance.trading.TradingEnvironment`
    """
    TRADING_ENV_FUTURE_CHAIN_PREDICATES = None
    MARKET_DATA_DIR = os.path.join(catalyst_dir, 'resources', 'market_data')
...
class WithSimParams(WithTradingEnvironment):
    ...
    See Also
    --------
    catalyst.utils.factory.create_simulation_parameters
    """
...
class WithEquityDailyBarData(WithTradingEnvironment):
    ...
    make_equity_daily_bar_data() -> iterable[(int, pd.DataFrame)]
        ...
        :func:`~catalyst.testing.create_daily_bar_data`

    See Also
    --------
    WithEquityMinuteBarData
    catalyst.testing.create_daily_bar_data
    """
...
class WithBcolzEquityDailyBarReader(WithEquityDailyBarData, WithTmpDir):
    ...
    - `cls.bcolz_daily_bar_ctable` is populated with data returned from
      ...
      :func:`catalyst.pipeline.loaders.synthetic.make_equity_daily_bar_data`.
...
class WithEquityMinuteBarData(_WithMinuteBarDataBase):
    ...
    catalyst.testing.create_minute_bar_data
    """
...
class WithFutureMinuteBarData(_WithMinuteBarDataBase):
    ...
    make_future_minute_bar_data() -> iterable[(int, pd.DataFrame)]
        ...
        :func:`~catalyst.testing.create_minute_bar_data`
...
class WithBcolzEquityMinuteBarReader(WithEquityMinuteBarData, WithTmpDir):
    ...
    - `cls.bcolz_minute_bar_ctable` is populated with data returned from
      ...
      :func:`catalyst.pipeline.loaders.synthetic.make_equity_minute_bar_data`.
...
class WithAdjustmentReader(WithBcolzEquityDailyBarReader):
    ...
    See Also
    --------
    catalyst.testing.MockDailyBarReader
    """
    @classmethod
    def _make_data(cls):
        return None
...
class WithSeededRandomPipelineEngine(WithTradingSessions, WithAssetFinder):
    ...
    seeded_random_loader : SeededRandomLoader
        ...
        catalyst.pipeline.data.testing.TestingDataSet.
    ...
    See Also
    --------
    catalyst.pipeline.loaders.synthetic.SeededRandomLoader
    catalyst.pipeline.loaders.testing.make_seeded_random_loader
    catalyst.pipeline.engine.SimplePipelineEngine
    """
